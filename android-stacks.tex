...
%In Java, stack trace consists of an ordered list of methods that were active on
%the call stack before the exception occurred.
...
checked, which means that the compiler statically checks if appropriate handlers are provided within the system. 
...
Both Java [ref: Gosling et al. 1996] and CLU [Liskov and Snyder 1979] support the checked exceptions,
 hence the compiler can then check if handlers are provided to such exceptions.
 However, this support is only partial because both languages also provides a form of unchecked exceptions.
In CLU,  if a handler is found for a given exception, it is converted in a predened default exception, 
called failure which is the only one which can be implicitly propagated [ref-clue].
 Java allows a hybrid approach on which the developer can define specific unchecked and checked exceptions,
and use both kinds of exceptions inside a single system.
In both languages the developer of a client module is not warned by the compiler if 
 unchecked exceptions are thrown by a module. 


Although exception handling mechanism has been embedded in several programming languages 
as show above,   the exception handling code is often generally poorly understood and the least
tested part of software systems. There is a long-lasting debate about the pros and cons of checked and unchecked exceptions 
~\cite{javatut,stackoverlow,debate},\footnote{152 questions in Stackoverlow
are related to this debate}. The debates, however, are based on the developers own experience in
using both models. There is a lack of empirical study tackling the pros and cons of each model.
...
To better understand the root causes reported on stacks, we categorized the top
100 distinct exception types in in one of the chategories listed on
Table~\ref{tab:categories}, based on the inspection of source code, Javadoc, or
additional documentation related to the exception. To ensure the quality
of the classification, three independent coders classified a randomly selected
...
We could observe that in 4 projects this strange wrapping happend 7 times in total.



\subsubsection{Error wrapping Checked Exception}

We could observe that in 7 projects this strange wrapping happend 8 times in total.
...
In one of the cases, however, in project OpenDocument.droid a java.lang.Exception was 
wrappings a java.lang.OutOfMemoryError.
This general handlers is a known bad practice and here we could observe a bad consequence of
such bad practice.
...
\begin{tabular}{llll}
    \hline
    \bfseries{Project} & \bfseries{Stars} & \bfseries{Watchers} & \bfseries{Forks} \\
    \hline
androidremocons &  9  &  2    & 10 \\
android-timepicker &  73  &  13    & 32 \\
PathfinderOpenReference &  23  &  12   & 15 \\
ActionBarSherlock &  5926  &   767  & 3356 \\
OpenDocument.droid &  40  &  9   & 12 \\
TouchDB-Android &  -  &  -    & - \\
ContractionTimer &  -  &  -    & - \\
\hline
  \end{tabular}
\caption{Projects containing wrappings (Error-Checked)}
\label{tab:wrappings2}
\end{table}
...
\subsubsection{Error wrapping Runtime Exception}
