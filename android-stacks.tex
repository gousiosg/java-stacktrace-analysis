...
(2) undocumented checked exceptions thrown by JNI interface;
...
the source code, and in the absence of the exception documentation it is very difficult or even impossible for the client of such third party code to 
...
During the manual inspection, besides inspecting the code of exception signalers we also
inspected the intermediate methods reported on the stacks.  The inspected stack traces envolved, 
in one way or another, methods from the  Android Application Framework: either framework 
utility methods, or framework hook methods  (e.g., OnCreate(), onPause()).
During this inspection we could observe that none of the hook methods provided by the Activity class 
specify an exception interface.  The``exception interface" can stimulate
can stimulate the developer (in this case, extending Activity class) to: think that 
``something" may go wrong during the execution of such methods; and design the 
exception handling code accordingly (the absence of such interface
 violates to best practice IV).

\noindent \fbox{ \begin{minipage}{0.96\columnwidth} \emph{
 None of the Activity hook methods define an ``exception interface".
}
\end{minipage}}
