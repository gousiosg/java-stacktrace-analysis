...
\begin{figure}
  \begin{center}
    \includegraphics[scale=0.5]{stack-size-wrappings-hist.pdf}
  \end{center}
...
when such exceptions are thrown by reused third party code (e.g. libraries, or framework utility code).
Since it is very difficult or even impossible for the client of such thrid party code to 
protect the application against undocumented runtime exceptions, the
...
As we can see from Table ~ref{typeroottab},  most of the exceptions signaled by third-party code
 (e.g., Android and libs) were runtime exceptions.

 Manually inspecting the library code of methods signaling runtime exceptions 
 - this analysis did not consider the exceptions implicitly signaled  by JVM (e.g. NullPoiterException) -
 we could also observe that none of the runtime exceptions signaled by them were documented.
Examples of such methos are listed on the Table X.

This result is aligned with the results of other study conducted by from Cabral
and Marcques ~\cite{} which ialso observe that runtime exceptions in .Net programs 
...
% the developer does not have access to their
...
%Manually inspecting the code signalers which RuntimeExceptions were thrown in Android
%Platform we could observe that direct instances of java.lang.RuntimeException
% were thrown due to two main reasons: (i) when there was no recovery action associated
%to the exceptional condition, which means that regardless the kind of exception the
%only possible action was to abort the application; and when translating exceptions
%signaled by application methods. Since Android platform is heavily based on
%call backs", when a application throws a runtime exception (i.e., one of the life cicle 
%methods of an Activity) and does it, the exception scapes to compotenst of the Android platform 
%which do not have contextual information to adequately handle
%the exception and simply terminates the application. 

%We could observe that although been known as a bad practice, the signaling general 
%exceptions in this context it was an intended design decision when no specific recovery 
%action can be defined for the exception.
