...
This paper reports a study which mined the exception stack traces included in
...
whether the analysis of stack trace information could reveal \emph{bug hazards} related to the exception handling code,
...
for the client code to protect against `unforeseen" situations that may happen while calling a third-party code.
...
exception signaled by native C code invoked by the Android Platform, yet this exception was not declared
 of the Java Native Interface invoking it. This can lead to uncaught exceptions that are 
difficult to debug. 
...
mined issues created by developers on GitHub and Google Code, while the previous works analyzed crash reports and automated test reports.
 On the other hand, our work points to bug hazards that were not detected by previous works (i.e., cross-type wrappings, undocumented checked exceptions and undocumented runtime exceptions thrown by third-party libraries) which represent new threats to the application robustness.
...
Hence, the study results are relevant to Android and Java developers who may underestimate the effect of such
 \emph{bug hazards} the application robustness, and how tricky it can be to prevent them. Moreover, such \emph{bug hazards} 
calls for improvements on languages and tools to better deal with them in Android and Java environments.
...
which also comprises (i) a set of native libraries written in C/C++ 
(e.g., WebKit, OpenGL, FreeType, SQLite, Media, C runtime library) - which 
...
- which include a subset of Java standard libraries and mainly “wrappers” to access the set of C/C++ 
native libraries using the Java Native Interface (JNI); (iii) the Dalvik runtime environment - 
which was specifically designed to deal with the resource constraints of a mobile device; 
...
(extends Error)~\cite{gosling2000java}. The checked exception received such name
 because they must be declared on the method's \emph{exception interface} (i.e., a list of exceptions that a method 
...
they do not need to be specified on method \emph{exception interface} and do not trigger any 
...
According to the Java Specification~\cite{gosling2000java} the programs are not 
expected to handle such runtime exceptions signaled by the runtime environment. 
The user-defined exceptions can be either checked
...
For simplicity along the text we will refer to exception stack trace as stack trace.
...
summarized below\footnote{We also compiled aditional guidelines related
to ways of handling exceptions but they are out of the scope of this paper.}:
...
also wise to document (either on the \emph{exception interface} or using @throws tag on Javadoc) 
the \emph{explicitly thrown} runtime exceptions as carefully as checked exceptions. 
The \emph{explicitly thrown} exceptions exclude the ones implicitly 
...
Such exceptions can lead to the uncaught exceptions - one of the main causes of 
systems crashes. 
