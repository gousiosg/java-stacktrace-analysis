...
\title{ What can GitHub and Googlecode Issues tell us about Exceptions in Android Apps? }
...
\subsection{Java Exception Handling Mechanism} \label{sec:extypes}
...
(extends Exception) - means that the compiler statically checks if appropriate handlers 
are provided within the system, to enable such checking each checked exception must be specified 
on the method signature;  the run-time exceptions (extends RuntimeException) and errors
(extends Error)~\cite{gosling2000java}  -  which do not need to be specified on method
...
\begin{figure} \centering \includegraphics[scale=0.]{stacktrace.png}
...
\label{sec:android}
 ...
 % SELECT id, exception, exception_extends, exception_from, exception_package, 
 %      issueid_onexminer, main_cause, main_extends, main_from, main_package, 
 %      main_pre_signaler_java, main_signaler, mainsignaler_class, mainsignaler_from, 
 %      mainsignaler_method, mainsignaler_package, pre_signaler_java, 
 %      repo, root_cause, root_extends, root_from, root_package, root_pre_signaler_java, 
 %      root_signaler, rootsignaler_class, rootsignaler_from, rootsignaler_method, 
 %      rootsignaler_package, signaler, signaler_class, signaler_from, 
 %      signaler_method, signaler_package, stack
 % FROM stacktracesummary;
...
%\begin{tabular}{llll}
%    \hline
%    \bfseries{Wrapper Type} & \bfseries{Root Cause Type} & \bfseries{Occurrences} & \bfseries{Projects} \\
%    \hline
      %none   & Runtime    & 359 &  2360  & - &  - \\
      %none  &   Checked   & 117 & 422 & - &  -   \\
      %none  &   Error    & 141 & 381 & - &  -  \\
      %none &  Undefined  & 10  &  15  & - &  -    \\
      %none & Throwable   & 1  &  1 & - &  -   \\
     %\hline
      %Runtime   & Runtime  & 182 & 560 & - &  -  \\    
      %Checked   & Checked   & 42 & 98  & - &  -  \\
      %Error     & Error    & 14 & 15  & - &  -   \\
    %\hline
