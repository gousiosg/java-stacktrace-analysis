...
Moreover, our work identified the type of each exception mined from issues
(classifying them as Error, Runtime or Checked) based on the source code
analysis of the exception hierarchy; we mapped the exception signalers (i.e., were the
exceptions signaled from libraries, the platform or the application itself); we investigated
the adoption of best practices based on the analysis of stack trace information.
Such analysis revealed intrigging cross-type exception wrappings which may bring the program 
to an unpredictable state (i.e.,  trying to handle an instance of OutOfMemoryError “hidden” in a checked exception).
