...
\begin{figure*} 
\centering \includegraphics[width=.9\hsize]{studyoverview3.png}
...
Then the stack traces embedded each issue are extracted and distilled (3).
...
a base exception type. Hence in this study the bytecode analysis was only used to discover the type 
of each mined exception when the jar file of such exception was available on the project or 
on the reused core Java libraries. An specific implementation (based on source code analysis)
 was needed to to discover the exception type when the bytecode was not available.
...
\finding{%
More than 50\% of the uncaught exceptions are due to errors in
...
from library code. Initial steps in thid direction have been proposed
...
which also used free or open-source Android apps~\cite{Linar13,McDon13,Ruiz12}.
