...
\subsection{Georgios's Introduction}
...
caught by the program's exception handling code, the program has the opportunity
to assess the conditions that caused the fault and either recover the
computation or abort. An exception that is not caught by an exception handler
leads to a program crash. In most programming languages with exceptions, a crash
usually causes debugging information to emited by the language runtime to help
the programmer identify and fix the reason of the crash. The debugging
...
tested part of software systems~\cite{coelho2011unveiling}. Similarily to source
code, current works focus their exception handling analysis on either static
(source code-based) or dynamic (runtime-based) analysis methods. Neither method
can explore the full execution space of a program and therefore they both fail
to account for exceptions caused by rutime issues (e.g. resource depletion) or
...
available data were only focused on few large scale projects. In the recent
past, many open source software projects have their source code and issue
repositories to Github. The concentration of projects and the existence of
projects that mine such information, such as GHTorrent~\cite{Gousi13}, made full
population studies possible.
...
subset of our initial sample (482 Android projects).
...
Some outcomes were consistently detected through
...
\subsection{Remaining Introductions :-)}
...
Bug issues have called the attention of researhc community and  have been the target of several 
...
%\subsubsection{Exception Chaining} Exception Wrapping 
