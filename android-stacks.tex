...
This paper reports a study which mined the Java stack traces included in
159,048 issues reported on Android projects hosted in GitHub (482 projects) 
and Google Code (157 projsects). The goal of this study is to investigate 
whether the analysis of stack trace information could reveal bug hazards related to the exception handling code,
 that may lead to a decrease in application robustness. 
 Overall 6,005 exception stack traces were extracted, and analyzed in 
combination with source code and bytecode analysis. 
Some of the outcomes of this study include the identification of the following 
bug hazards: (i) unexpected exception wrappings (such as Errors being wrapped in checked exceptions) -
indicating that Java's hybrid exception model become fault-prone when it is not fully used in 
accordance with its purpose; (ii) undocumented runtime exceptions thrown by both the Android 
platform and third party libraries; and (iii) undocumented checked exceptions thrown by the Android Platform.
This study provides insights on such bug hazards and the robustness threats  
they impose not only to Android apps but also to other systems based on Java exception model.
...
The goal of this study is to investigate whether the reported exception stack traces
 can reveal common bug hazards in the exception-related code.  
A bug hazard  ~\cite{binder2000testing}  is a circumstance that increases the 
...
platform and Android applications. Some bug hazards consistently detected 
...
Trying to handle an instance of OutOfMemoryError ``hidden" in  a checked exception may bring the program
 to an unpredictable state. Such wrappings suggest that, when not adequately used, the Java's hybrid exception
 model may become fault-prone.
...
exceptions may remain uncaught and lead to system crashes. 
   ...
   \item Undocumented checked exceptions signaled by native C code.  Some flows reported a checked 
exception signaled by native C code invoked by the Android Platform, yet this exception was not declared
 of the Java Native Interface. This can lead to uncaught exceptions that are 
difficult to debug. 

 \item A multitude of programming mistakes - approximately 52\% of the reported stack traces can 
be attributed to programming mistakes. In particular, 27.71\% of all stack traces reported a java.lang.NullPointerException 
as its root cause.

\end{itemize}

 The high prevalence of NullPointerExceptions and the undocumented runtime exceptions signaled by the Android Platform
 is aligned with the findings of other research works ~\cite{kim2013predicting,fraser20131600,csallner2004jcrasher} and 
~\cite{kechagia2014} respectively - which detected such bug hazards while testing or mining crash reports of Java and Android projects. 
On the one hand, the findings our study emphasize the impact of these bug harzards on the application robustness 
by mining a different source of information: the issues created by developers on GitHub and Google Code. On the other hand, 
our work points to bug hazards that were not detected by previous works (i.e., cross-type wrappings, undocumented checked exceptions and 
undocumented runtime exceptions thrown by third-party libraries) which can also negatively affect the application robustness.
...
Hence, the study results are relevant to Android and Java developers who may underestimate the effect of such
 bug hazards the application robustness, and how tricky it can be to prevent them. Moreover, such bug hazards 
calls for improvements on languages and tools to deal with such bug hazards in Android and Java environments.
...
By convention, instances of Error represent  unrecoverable conditions which usually results
from failures detected by the Java Virtual Machine due to resource limitations such as OutOfMemoryError;
normally they cannot be handled inside the application.  

Instances of RuntimException are implicitly thrown by Java runtime environment when a program violates 
the semantic constraints of the Java programming language (e.g., out-of-bounds array index, divide-by-zero 
error, null pointer references). Some programming languages react to such errors by peremptorily terminating the program; 
...
The programs are not expected to handle such runtime exceptions signaled by the runtime environment according 
to the Java Specification ~\cite{gosling2000java}. 

The user-defined exceptions can be either checked or runtime. There is a long-lasting debate 
...
a set of best practices related to each exception type.
...
the explicitly thrown runtime exceptions as carefully as checked exceptions. 
When mentioning reffering to `` explicit" thrown exceptions
we  exclude the implicitly signaled runtime exceptions related to programming
mistakes. Such practice is mostly needed when developing library/framework 
code reusable code - as the developers do not have access to the source code. 
Doing so, the clients of such library/framework
...
of exception stack traces can pinpoint best practices violations and therefore reveal bug hazards.
As mentioned before, in this context ``bug hazards" are the characteristics on the exception-related code 
...
Manual inspections were also used to support this mining process (Figure~\ref{fig:overview} - (4), (7) and (9)) and support
the search for bug hazards performed in this study  (Figure~\ref{fig:overview} - (8)). Next sections detail each step of this mining process.
...
projects analyzed - and therefore can review bug hazards on the exception-related code -  
we opted for not restricting the analysis only on defect issues\footnote{We conducted the same analysis 
...
found niether automatically, nor manually (because they were defined on a previous version
...
quantitative and qualitative analysis of the outcomes. The structure of this section
 is centered on questions that guided the search for bug hazards on the exception-related code
...
refers to a programming mistake according to its Javadoc; and (ii) the Java specification list all exceptions related to backward compatibility
...
\emph{\textbf{Inspecting Exception Interfaces.}}  According to the best practices mentioned before, runtime exceptions 
should be documented as part of the exception interface of libraries/framework reusable
...
runtime exceptions in .Net programs are most often not documented; and 
...
of the work of Kechagia and Spinellis is that it did filter out exceptions that should 
although unchecked not be documented -  because they are implicitly signaled by the 
JVM due to resource restrictions or violations on Java semantic constraints.

The ``undocumented runtime exceptions" that are explicitly siglened represent a threat to system robustness, specially
...
\noindent \fbox{ \begin{minipage}{0.96\columnwidth} \emph{
The study revealed: 267 exception stack traces were caused by ``undocumented runtime exceptions" thrown by libraries or the Android platform; 
several methods defined on 11 android packages signaling direct instances of RuntimeException.
}
\end{minipage}}
...
on the ``exception interface" of such method and all methods on the invocation chain.
}
\end{minipage}}
...
Table~\ref{tab:exampeswrap} exemplifies some of the wrappings performed in this code snipet:  
an instance of RuntimeExceptions wrapping instances of OutOfMemoryError and StackOverflowError. 
Such wraps mask an unrecoverable error wrapping on a general runtime exception.

Moreover, such wrapping also pointed out to another best practice violation: the methods from Android platform
signaling direct instances of RuntimeException. Using such general exception, is considered a bad practice 
according to Java specification and compiled guidelines, because it loses contextual information about the exception.
...
%:
%\emph{``I call it my billion-dollar mistake. It was the invention of the null reference in 1965 [...] 
...
pinpoint bug hazards on the exception-related code based on 
...
and third-party libraries.  Additionaly,  the ``bug hazards" observed in this study are due to
...
bug hazards such as the cross-type exception wrappings not discussed in previous works.
...
to identify common bug hazards on the exception related code that can lead to 
...
stack traces to pinpoint bug hazards on the exception-related code. 
In this study, bug hazards are characteristics on the exception-related code that favor the introduction
of  uncaught exceptions and unintended handling - which can contribute to 
degrade the robustness of applications.
Some bug hazards consistently detected in this study were: 
...
The bug hazards discussed in this work can negatively affect not only the robustness of Android application 
