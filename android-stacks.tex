...
Trying to handle an instance of OutOfMemoryError ``hidden" in  a checked exception may bring the program
 ...
 model may negatively impact the application robustness.
...
\end{itemize}
 ...
 The high prevalence of NullPointerExceptions is aligned with the findings of other research works ~\cite{kim2013predicting,fraser20131600,csallner2004jcrasher} as well as the undocumented runtime 
exceptions signaled by the Android Platform ~\cite{kechagia2014}. On the one hand, some of the findings of our
 study emphasize the impact of these bug harzards on the application robustness 
by mining a different source of information as the ones used in previous works. The present work 
mined issues created by developers on GitHub and Google Code, while other works analyzed crash and automated test reports.
 On the other hand, our work points to bug hazards that were not detected by previous works (i.e., cross-type wrappings, undocumented checked exceptions and undocumented runtime exceptions thrown by third-party libraries) which can also negatively affect the application robustness.
...
normally they cannot be handled inside the application.  Instances of RuntimException are implicitly 
...
to the Java Specification ~\cite{gosling2000java}. The user-defined exceptions can be either checked
 or runtime. There is a long-lasting debate 
...
to identify bug hazards on the exception handling code of Android apps.
