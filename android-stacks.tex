...
on 30,08\% of all analyzed exception stack traces, and ewas mentioned in approximatelly 
50% of all projects analyzed. Moreover,  the cross-project analysis 
of exception stack traces enabled to consistenly reveal   
fragilities of exception handling code: 
(1) undocumented runtime exceptions signaled by third party code,
(2) undocumente checked exceptions thrown by Android native code
...
Such fragilities impose challenges to application robusness not only 
in Android environemt but also to other systems based on Java exception model.
...
the mobile industry shiped over 315 million smartphones in the last quarter of 2013 ~\cite{googleio}.
...
In this context, Android has become a leading plaftorm for such applictions - thanks to its open-source 
...
they have to face new threats to application robustness imposed by mobile environmet.
...
on the way exceptions on JNI interface funcions, which can lead to uncaught exceptions that are difficult do 
debug.  

  \item  A multitude of programming mistakes were reported as the main causes of stack traces:
 XXX  of reported stack traces were caused by programming mistakes; and XXX of analyzed stack traces reported a java.lang.NullPointerException as root cause;
...
the static blocks); and the undocumented runtime exceptions were also found in Java libraries besides the Android plaftorm.
...
background on Android Plaform and Java exception model. 
...
the APIs to support concurrent developement (e.g. AsyncTask). 
...
unresponsivedue due to resurce limitation and excessive network access).
...
can pinpoint best practices violations. Chraracteristics of exception stack traces and detected violations can reveal fragilities
...
was retrieved from GitHub and Googlecode, how the exceptions stack traces were extracted and distilled,
 and how the type associated to each exception and its origens was identified.
...
projects analyzed - and therefore can review fragilities on the exceptio related code -  
...
Different from GitHub, Googlecode does not provide an API to access the information related
 to hosted projects \footnote{Googlecode previoulsly provided a webservice but was deacrivated in June 2013.}.
However many well used open source Android application such as K9mail, is hosted in Googlecode which stores
the issue history of this and other apps are store there ~\footnote{K9mail moved to Github but advice their users to create issues on Googlecode as a way of not loosing the issue history.}.

To overcome this limitation and mine the information available on such Googlecode 
projects we implemented a Web crawler which navegated (incorporated in Exception Miner tool described next) 
 on each project and issue page, extracting the content of each issue to store on relational database. \footnote{Google code previoulsly provided a webservice to extract information about each project, but this service was removed in what Google called "clean-up action" which happened in June 2013.}

To overcome this limitation we needed to implement a Web Crawler (incorporated in Exception Miner tool described next) that navegated 
...
reporting at least one stack trace, and as a result we could identify 158 
Android projects.
...
to connect to various repositories (such as Google Code,
GHTorrent or even directly to Bugzilla), extract issues, mine stack traces from
them and distill exteption stack trace information. The main components of
ExceptionMiner are as follows:

\noindent\emph{Repository Connectors} The first step in the process is
...
This module analyzed the exception hierachy of all exceptions thrown on:
...
and this module extractes the packages hierarchies (recursive list of 
...
and signalers origin (e.g., root exception and its type, root signaler and its origin,
exception wrapper type, wrapper signaler).
...
%We then analyzed the lables associated to such issues, and we discovered that only 5,196 were
...
This study was also based in a set of manual inspection tasks. Besides the 
 identification of Android projects detailes previously. 
...
This section presents the results amed at answering the research 
questions presented before.

\subsection{The Common Root Exceptions and What They can Tell us? }
...
Table \ref{tab:toptenandroid} illustrates the top 10 root couses found in the study,
 ranked by the number of distinct project repositories on which they were reported. 
Thes table shows: the number of times each exception was reported as 
...
part of Android, the aplication itself, libcore, or a third-party library as presented in Table  \ref{tab:signalers} 
 \footnote{9 stack traces contained only elements of JVM and were discarded from the analysis}.
...
which are mostly not expliciltly thrown by the developer but include
...
(which represents 20,54\% of all projects reporting at least one stack trace).
 ...
 The NullPoiterExceptions are mainly signaled inside Android platform and inside 
...
practice III. It is considered a bad practice because exception type does not carry enough
 information to identify the cause of the exceptional behaviour, and as a consequence developers need to relying on
...
Such categorization was based known sources of faults mobile developement: concurrency ~cite{to_include}
, backward compatibility ~cite{to_include}, security  ~cite{to_include} and resource management (IO, Memory, Batery) ~cite{to_include}.
...
\begin{table}
  ...
  \begin{tabular}{lrr}
    \hline
    \bfseries{Category} &  \bfseries{Occurence on Stacks (\%)} \\
    \hline
      Programming logic (java.lang and util) & 2326 (57,9\%)\\
      Resources (IO)                         &  727 (18,1\%) \\
      Security                               &  165 (4,1\%)\\
      Concurrency                            & 116  (2,9\%)\\
      Backward compatibility                 &  219 (5,5\%) \\
      Specific Exceptions               &  197 (4,9\%)\\
      General (Error, Exception, Runtime)    &  267 (6,7\%)\\
    \hline
  \end{tabular}
...
resources are usualy scarse). We could observe that there were 
stack traces related to the main concerns of mobile develoment.
...
In order to evaluate if  best practice IV was being obeyed, we manualy inspected the   
...
A checked exception can remain uncaught if it is declared by evey method on the call chain or if 
it is wrapped in a runtime exception and rethrown. Next, section describes the kinds of 
wrappings found in this study.

\subsection{What the Cross-Type  Exception Wrappings can Tell us?}
...
The most frequent exception wrapping were were performed inside RoboSpice
 is an android library that supports the creation of asynchronous long running tasks.
This library has 1.682 stars and 367 forks.

Most of such wrappings however were caused by the reflection Java library, used by Android, 
Application and library classes. Reflaction methods convert every exception thata happens 
when invoking a reflected method into an InvoncationTargetException (this conversion is performed inside native code).
