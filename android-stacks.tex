...
\emph{II-Error represents an unrecoverable condition detected by the runtime enviroment which
...
on Exception miner. Next, we present core analyses which were implemente on ExceptionMiner. Such analyses 
...
\emph{\textbf{The prevalence of programming mistakes.}} To get a broader view of the prevalence of
programming mistakes of reported exception stack traces we: (i) identified the Java exceptions that can be
implicitly thrown by the runtime environment due to a programing mistake or resource limitation; and (ii)
 used ExceptionMiner to evaluate the prevalence of such exceptions on the reported stack traces.
Table ~\ref{tab:causes} illustrates the main causes of exceptions reported on issues.

\begin{table}
  \centering
  \begin{tabular}{lrr}
    \hline
    \bfseries{Cause} &  \bfseries{\% Occurrences on stacks} \\
    \hline
      Programming logic (java.lang and util) &  52,0\%\\ 
      Resources Restriction   &   23,9\% \\ 
      General Exceptions (Error, Exception, Runtime)    &  6,7\%\\
      Specific  &  xxx\%\\ 
 \end{tabular}
...
The analysis on the complete set os stack traces revealed that, in fact, the programming mistakes were
the most reported root causes.  Although there is usually no proper handling for such exceptions, 
...
This finding calls the attention to the fact that: high number of programming errors reported 
on issues may me an indication that developers are underestimating the impact that such 
...
from Table ~\ref{tab:typeroottab},  most of the reported exceptions signaled (explicilty or implicitly by the runtime enviroment) on methods defined
...
signaled by  runtime environment (due to programming mistakes) 
...
detected by the runtime environment (e.g.,  OutOfMemoryError). On the other hand, checked exceptions should
...
Such exceptions are implicitly thrown by runtime environment, and among them the NullPointerException 
