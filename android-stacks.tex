...
The exception (mis-)use patterns that emerged  in this study sheds enough light on 
problems that may be causeing the lack of rocustness of current Java programs in the face of
exceptional conditions: (i) the undocumented runtime exceptions thrown by library code - 
...
scaping from library code. (ii) the odd wrappings found in this study can lead to what we call
an \emph{exception handling confusion problem} which may lead the problem to an
unpredictable state while trying to handle an exception. To illustrate this problem, we can use
...
by the JVM which make impossible the program to recover. Further investigation
is needed on finding ways to help developers in deadling with such problem, either preventing 
odd wrappings or enabling the developer to better dealing with them - when they are innevitable. 
