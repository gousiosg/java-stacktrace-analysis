...
and Google Code (157 projsects). 
The goal of this study is to investigate  whether the exception handling logic on these stack traces
 can reveal common characteristics on the exception-related code that
are particularly fault prone and may lead to a decrease in application robustness.
Overall 6,005 exception stack traces were extracted, and analyzed in 
combination with source code and bytecode analysis. 
Key problems identified in this study include: 
...
(3) unexpected exception wrappings (such as Errors being wrapped in checked exceptions),
indicating that Java's hybrid exception model is not fully used in accordance with its purpose.
Moreover, about 50\% of all analyzed projects reported at least one NullPointerException
as the root cause of an exception stack trace, suggesting the presence of programming mistakes.
...
In particular, 27.71\% of all stack traces reported a \texttt{java.lang.NullPointerException} as its root cause.
...
trace on which the NullPoiterException was the root cause.
 ...
 The NullPointerException was mainly signaled inside the applicaiton code (50\%) and the Android platform (31,5\%),
...
The high prevalence of NullPointerException is aligned with the findings of other 
...
documentation hold enough information to pinpont a ``programming mistake".To exemplify, an instance of ArrayOutOfBoundException
...
\noindent \fbox{ \begin{minipage}{0.96\columnwidth} \emph{
The study revealed: an unexpected fragility - a checked exception being thown by a native method and not declared  
...
\emph{\textbf{Runtime Exception wrapping Error.}} We could observe that 44\% of the wrappings were performed 
...
(window) of an Android app. This class contains a such try-catch block that
handled any instance of java.lang.Exception signaled in the context 
of the method in a general Runtime exception and re-throws. 
