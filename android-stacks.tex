...
Natal, Brazil\\
Email: \texttt{roberta@dimap.ufrn.br,lucas.almeida@lcc.ufrn.br}}
...
mechanism, embedded in many main
...
and prints the exception stack trace. Figure~\ref{fig:wrapping} presents an 
example of a stack trace, on which multiple 
exception chaining is depicted. Exception Chaining (or exception wrapping) is  
common way of propagating exceptions  across components of different 
abstraction levels. It is implemented by special exception 
constructor which receives another exception as a parameter. 
...
bloch2008effective}. 
% To guide our investigation, we compile the guidelines
A compiled list of the guidelines related to exception types and exception propagation in Java the following:
...
todo{RSC:this practice was not well explored, can be removed if we need space.}

%\noindent\emph{Exception Documentation}
\textbf{V-Document all exceptions thrown by a
...
\subsection{Direct instances of RuntimeExceptions been thrown.}
...
\subsection{Undocumente runtime exceptions been thrown.}

Manualy inspecting the code of the methods throwing direct instances of RuntimeException revealed 
another and even worse bad practice:  none of the analyzed methods 
documented this exception as part of the exception interface (e.g., using @throws annotation).
...
protect the application against undocumented runtime exceptions. As a consequenve, the
...
root exception. Table~\ref{typeroottab} presents the types of root causes of all stack traces reported on 
the Android repositories. As we can see from Table ~ref{typeroottab},  most of the exceptions signaled by third-party code
 (e.g., Android and libs) were runtime exceptions. If we filter the exceptions implicitly signaled by 
JVM (due to coding erros) the number of runtime exceptions thrown by third party code is XXX 
signaled by Android and YYY signaled by library code.
...
Manually inspecting 10% of each set we could also observe that none of the methods throwing 
runtime exceptions documented the exceptions on the method signature (as suggested by Boch
 ~\cite{}) nor included a comment in Javadoc. This result is aligned with the results of other 
study conducted by from Cabral and Marcques ~\cite{} which  observed that 
runtime exceptions in .Net programs are most often not documented.

On the other hand, we could observe that the checked exceptionsmost represent most of the  
exceptions signaled by the set of libraries reused by Android platform (i.e., org.apache.harmony,
org.w3c.dom, sun.misc, org.apache.http, org.json, org.xml, javax.) referred in
this work as libcore. It is pointed as a good practice
...
\begin{figure}
  \begin{center}
    \includegraphics[scale=0.5]{only-wrap-histogram.pdf}
  \end{center}
  \caption{Stack sizes (top) and wrappings (bottom) for Java (left) and Android (right) stack traces}
  \label{fig:sizewrapbhists}
\end{figure}

We could also observe that in 205 stacks found in Android repository the exception was wrapped 
more then once. Figure ~\ref{sizewrapbhists}  illustrates the number of wrappings per stack, both
 in Java and Android studied repositories. One interesting wrapping found in ones Android repo was the 
