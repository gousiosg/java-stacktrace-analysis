...
\title{Unveiling Exception Handling Bug Hazards in Android based on GitHub and Google Code Issues }
...
%(e.g., faults in underlying middleware or hardware; memory and batery restrictions). 
...
This section presents the results of the study, this presentation is centered around parts 
of exception stack traces (i.e., the root exceptions, the exception types, 
and the exception wrappings).  Each part is then analyzed in detail to 
check whether they can reveal bug harzards on the exception handling code
related or not to violations of the best practices presented
 in Section~\ref{sec:best}.
...
This analysis revealed that the exceptions implicitly thrown by runtime environment 
due to mistakes on programming logic (e.g., out-of-bounds array index, null pointer 
references) and resource limitations represent a major bug hazard to the exception 
handling code of Android applications - security ..... os outros nao estao causando 
muitos problemas.

Although such exceptions do not violate the best practices described before (which are related to the 
explicitly thrown exceptions), they represent a major threat to apps robustness.
...
\finding{Almost two thirds of all crashes come from run time
...
\finding{%
Only a small fraction (4\%, 267 stack traces) of run time exceptions are programmatically
thrown. Almost none (0.4\%, just one) of these were documented.}
...
interface of these methods signaling them. 

The native method in question was defined in
...
not perform static exception checking on native methods. 

This type of bug is hard to diagnose
...
\subsection{Exception Wrappings}
...
They violate the semantics of Java's original exception
design (e.g., when mapping unrecoverable Errors to other types of
exceptions),
and may lead to lengthy exception chains.
}
