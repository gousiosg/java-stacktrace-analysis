...
Both Java [ref: Gosling et al. 1996] and CLU [Liskov and Snyder 1979] support the declaration of exceptions
 in method signatures, hence the compiler can then check if handlers are provided to such exceptions.
...
called failure which is the only one which can be implicitly propagated in this model [ref-clue].
 Java allows a hybrid approach on which the developer can define specific exceptions of both kinds 
(checked or unchecked) to be thrown and handled inside the system.
Therefore, in both languages the developer of a client module is not warned by the compiler of the possibility of 
an unchecked exception being thrown. 
