...
In this case, the exception condition masks an unrecoverable error,
which makes the task of providing a proper handler
infeasible.\footnote{TODO: Please note that Errors are not supposed to be
caught. The main problem here is that the program continues with e.g. no
memory available.}
...
\textcolor{red}{Since our aim is to learn from a large number of developers, we
use surveys which are known to scale well.}
\textcolor{red}{The survey is split into two logical sections; the motivations behind each logical section are:
(i) questions aiming at learning from developers about the usage of the exception handling code in app development;  and
(ii) questions focusing on getting developers perceptions about the bug hazards detected in the first phase of this study.
...
\textcolor{red}{The process to
identify the type of exceptions reported in stack traces may not be completely
accurate.
