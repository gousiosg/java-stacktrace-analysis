...
can lead to a scenario where the developer tries to handle such exception, leading the program
to na unpredictable state. This kind of wrapping should be avoided, since represent a
serious bug hazard.}
...
\textcolor{red}{By confronting the API user with a checked exception,
the API designer is forcing the client to handle the exceptional condition. 
But in this case the exception condition in and unrecoverable error,
which makes infeasible the task of providing a proper handler. 
This kind of wrapping is even more dangerous the previous one
...
\textcolor{red}{Since our aim is to learn from a large number of developers, we use surveys which scale well.}
...
\textcolor{red}{Another internal threat to validity of this study regards to the process adopted to 
...
Some of such exploratory surveys were applied to Android developers  [][][][]. Despite the low response rate
of all these surverys (between 4\% and 2\%), these studies could get interesting insights about the state 
of practice of Android development. Kochhar et al conducted a survey-based study to discover the 
...
Joorabchi et al. performed a survey-based study whose goal was to better understand the the main challenges 
...
response rate of the survey could not be calculated. Linares-Vásquez et al. surveyed 485 open source Android 
...
Bavota at al. investigated the impact of API change-proness and fault-proneness on the user ratings of Android apps. 
...
None of these surveys, however, assed the how developers deal with exceptions in Android nor 
the developers' perspective regarding a set of exception handling bug hazards. Our survey
was the first exploratory study focusing on exception handling issues in Android development.
...
Kochhar et al conducted a survey-based study whose goal was to discover the commonly used tools for mobile app testing as well as the problems faced by developers while testing the apps. The survey was sent to 3,905 emails of Android developers and received 83 responses (response rate of 2.13%). 

- M. E. Joorabchi, A. Mesbah, and P. Kruchten. Real challenges in mobile apps. In
...
This was the first exploratory survey study whose
goal was to assess developers' perspective regarding a set of exception handling bug hazards
as well as how developers deal with exceptions and prevent crashes while developing.
